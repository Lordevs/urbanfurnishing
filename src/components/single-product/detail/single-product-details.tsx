      ...
      <div className="flex flex-col lg:flex-row gap-12 xl:gap-20">
        ...
        <div className="w-full lg:w-[45%] flex flex-col">
            ...
            <div className="flex items-center gap-0.5">
              {[...Array(5)].map((_, i) => (
                <Star
                  key={i}
                  className={`w-4 h-4 ${
                    i < 4 ? "fill-[#1A1A1A] text-[#1A1A1A]" : "fill-transparent text-[#EBEBEB]"
                  }`}
                />
              ))}
            </div>
            <span className="text-[14px] text-[#666666] font-medium">
              <span className="text-[#1A1A1A] font-bold">4.8</span> (127 reviews)
            </span>
          </div>

          <p className="text-[#666666] text-[15px] sm:text-[16px] leading-relaxed mb-8">
            Experience Italian luxury with our meticulously curated Milano Collection. This comprehensive package brings together timeless elegance and contemporary comfort, featuring hand-selected pieces that transform your living space into a sophisticated sanctuary.
          </p>

          {/* Price Box */}
          <div className="bg-[#FAFAFA] border border-[#F2F2F2] rounded-[20px] p-6 lg:p-8 mb-8">
            <div className="flex items-end gap-3 mb-2">
              <span className="text-3xl lg:text-[36px] font-bold text-[#1A1A1A] leading-none tracking-tight">
                AED 23,999
              </span>
            ...
            <p className="text-[#C9A76A] text-[14px] font-medium">You save AED 9,080</p>
          </div>

          {/* Quantity */}
          <div className="mb-8">
            <span className="block text-[14px] font-medium text-[#1A1A1A] mb-3">Quantity</span>
          ...
          <div className="flex flex-col gap-3 mb-10">
            ...
            <Button className="w-full h-[54px] lg:h-[60px] bg-white border-2 border-[#EBEBEB] hover:border-[#412A1F] text-[#1A1A1A] rounded-[14px] text-[15px] font-bold flex items-center justify-center transition-all duration-300 shadow-sm hover:shadow-md cursor-pointer hover:-translate-y-0.5">
              Buy Now
            </Button>
          </div>
          ...
          {/* Key Benefits */}
          <div className="mb-10">
            <h4 className="text-[15px] font-bold text-[#1A1A1A] mb-5">Key Benefits</h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {benefits.map((benefit, idx) => (
                <div key={idx} className="flex items-center gap-3">
                  <benefit.icon className="w-5 h-5 text-[#888888] stroke-[1.5]" />
                  <span className="text-[14px] text-[#444444] font-medium">{benefit.text}</span>
                </div>
              ))}
            </div>
          </div>
              ...
              {["Overview", "Specifications", "What's Included"].map((tab) => (
                <button
                  ...
                  className={`relative pb-4 text-[14px] lg:text-[15px] font-medium transition-colors cursor-pointer ${
                    activeTab === tab ? "text-[#1A1A1A]" : "text-[#888888] hover:text-[#1A1A1A]"
                  }`}>
                  {tab}
                  {activeTab === tab && (
          ...
          <div className="flex flex-col gap-3">
            {features.map((feature, idx) => (
              <div
                ...
                <span className="text-[14px] text-[#444444] font-medium">{feature}</span>
              </div>
            ))}
          </div>
        </div>
