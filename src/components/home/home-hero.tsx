      ...
      buttons={
        <>
          <PrimaryButton
            ...
            label="Get in Touch"
            mbLabel="Get Started"
            className="rounded-xl sm:rounded-full bg-[#C3A16A] sm:bg-[#3D261C] hover:bg-[#b08e58] sm:hover:bg-[#2C1A11] text-sm py-3 h-auto"
          ...
          <SecondaryButton
            ...
            className="w-full sm:w-auto rounded-xl sm:rounded-full bg-transparent sm:bg-white/70 border border-white/50 sm:border-none backdrop-blur-md text-white sm:text-[#412A1F] hover:bg-white/10 sm:hover:bg-white/90 h-auto py-2.5 md:px-6 md:py-4 md:h-max md:text-base font-medium sm:font-semibold transition-all duration-300 shadow-lg shadow-black/5 hover:-translate-y-0.5 hover:shadow-xl hover:shadow-black/10"
          />
        </>
      }
      bottomCard={
        <motion.div
          ...
          className="md:absolute bottom-4 right-4 sm:bottom-0 sm:right-0 z-0 bg-[#F9F9F9] sm:bg-white px-12 sm:px-6 rounded-[24px] sm:rounded-tl-[32px] sm:rounded-br-[24px] sm:rounded-bl-none sm:rounded-tr-none py-4 sm:py-12 2xl:py-14 flex justify-evenly sm:justify-start items-center gap-6 sm:gap-14 shadow-xl sm:shadow-none w-auto sm:w-auto mx-auto mb-5 sm:mb-0"
        >
          {stats.map((stat, index) => (
            ...
            >
              <span className="text-[28px] sm:text-4xl leading-none font-bold sm:font-medium text-[#412A1F]">
                {stat.value}
              </span>
